...
public class RedoUndoTEMPPPPPPPPPPP {

	static Vector <Vector<GraphicComponent>> history = new Vector <Vector<GraphicComponent>>();
	static Vector<GraphicComponent> canvas = new Vector<GraphicComponent>(); 
	...
	private static void addCopySaveInHistory() {
		...
		history.add(temp);
		canvas.add((GraphicComponent)DeepClone.clone(CanvasGC.getCanvas()));
	}
	...
	private static void changeNow() {
		...
		CanvasGC.setCanvas(canvas.get(nowHistoryNum));
	}
